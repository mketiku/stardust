...
import com.stardust.dto.DockingBayResponse
import com.stardust.dto.DockingManifestResponse
...
class DockingController(
    ...
    fun getActiveDocks(): List<DockingBayResponse> = dockingService.getActiveDocks().map { mapToBayResponse(it) }
    ...
    fun authorizeDeparture(
        @PathVariable id: Long,
    ) {
        dockingService.departStation(id)
    }

    private fun mapToBayResponse(bay: com.stardust.domain.model.DockingBay) =
        DockingBayResponse(
            id = bay.id!!,
            bayCode = bay.bayCode,
